...
namespace QuadTree.Common
{
    public abstract class QuadTreeNodeCommon<T, TNode, TQuery> where TNode : QuadTreeNodeCommon<T, TNode, TQuery>
    {
        ...
        /// </summary>
        internal Vector2 Subdivide(bool recursive = true)
        {
            ...
            Vector2 mid = new Vector2(Rect.X + (Rect.Width / 2), Rect.Y + (Rect.Height / 2));
        ...
        /// </summary>
        public void Subdivide(Vector2 mid, bool recursive = true)
        {
        ...
        private UInt32 MortonIndex2(Vector2 pointF, float minX, float minY, float width, float height)
        {
            pointF = new Vector2(pointF.X - minX, pointF.Y - minY);
        ...
        protected abstract Vector2 GetMortonPoint(T p);

        internal void InsertStore(Vector2 tl, Vector2 br, T[] range, int start, int end, Func<T, QuadTreeObject<T, TNode>> createObject)
        {
            if (ChildTl != null)
            {
            ...
            if (count > 8 && area > 0.01f && !float.IsInfinity(area))
            {
                ...
                Vector2 middlePoint = GetMortonPoint(range[quater2]);
                ...
                ChildTr.InsertStore(new Vector2(middlePoint.X, tl.Y), new Vector2(br.X, middlePoint.Y), range, quater1, quater2, createObject);
                ChildBl.InsertStore(new Vector2(tl.X, middlePoint.Y), new Vector2(middlePoint.X, br.Y), range, quater2, quater3, createObject);
        ...
        public void AddBulk(T[] points, Func<T, QuadTreeObject<T, TNode>> createObject)
        {
            ...
            InsertStore(QuadRect.Location, new Vector2(QuadRect.Bottom, QuadRect.Right), range, 0, range.Length, createObject);
        }
        ...
        where TNode : QuadTreeNodeCommon<T, TNode>
    {
        ...
        {
            return search.Intersects(rect, this.Rect);
        }
    }

    public abstract class QuadTreeNodeCommonPoint<T, TNode> : QuadTreeNodeCommon<T, TNode, Vector2>
        where TNode : QuadTreeNodeCommon<T, TNode, Vector2>
    {
